...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/05/08 18:03:47 by ikryvenk          #+#    #+#             */
/*   Updated: 2017/05/08 18:15:15 by ikryvenk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		initialize_sets(t_field *field)
{
	...
	while (i < field->ways_quantity)
	{
		field->set_ways[i] = (int*)malloc(sizeof(int) *
				(field->max_path_in_way + 1));
...
static int	compare(t_field *field, int *set, int *x)
{
	...
	while (set[i] != -1)
	{
		...
		if (field->arr_ways[set[i]][0] == x[0] &&
			field->arr_ways[set[i]][1] == x[1] &&
